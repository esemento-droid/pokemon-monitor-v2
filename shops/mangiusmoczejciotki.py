...
SHOP = "mangiusmoczejciotki"
BASE = "https://mangiusmoczejciotki.pl"
CAT_URL = BASE + "/pl/c/Pokemon-TCG/662"
HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36"}
EXCLUDE = [
    "sleeves", "koszulk", "toploader", "album", "pro-binder", "ultra pro", "ultra-pro", "portfolio",
    "academy", "playmat", "energia", "energy", "wejsc", "turniej", "cup", "battle deck",
    ...
    "chińsk", "chinese", "(chi)", "s-chinese", "segregator", "deck box", "alcove", "lorcana",
...
def parse_page(html):
    ...
    for tile in soup.select("product-tile"):
        ...
        products.append({"id": f"mangiusmoczejciotki_{pid}", "name": name, "price": price_str, "shop": SHOP, "url": href, "image": image, "stock": None, "available": available})
    return products
...
async def get_products():
    ...
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        for a in soup.select("a[href]"):
            m = re.search(r"/662/(\d+)", a.get("href", ""))
            if m:
            ...
            if page_html:
                for p in parse_page(page_html):
                    if p["id"] not in seen_ids:
                        seen_ids.add(p["id"])
                        products.append(p)
    print(f"[MANGIUSMOCZEJCIOTKI] {len(products)} produktow")
    return products

...
CATEGORIES = [
    "https://grybezpradu.eu/pokemon-tcg",
...
EXCLUDE = [
    ...
    "japonsk", "japanese", "japan", "korean", "koreańsk", "ultra pro", "toploader", "sleeves",
    ...
    "playmat", "portfolio", "pro-binder", "album", "koszulk", "segregator", "deck box", "alcove",
...
async def get_category(session, base_url):
    ...
    tiles = []
    for i, h in enumerate(all_htmls):
        s = BeautifulSoup(h, "lxml") if i > 0 else soup
        tiles.extend(s.select("product-tile"))
    return tiles
...
async def get_products():
    ...
    for tile in all_tiles:
        ...
        if image and not image.startswith("http"):
            image = f"https://grybezpradu.eu{image}"
        link = tile.select_one("a")
        url_prod = ""
        if link:
            url_prod = link.get("href", "")
            if url_prod and not url_prod.startswith("http"):
                url_prod = f"https://grybezpradu.eu{url_prod}"
        products.append({
            "id": f"grybezpradu_{pid}",
            "name": name,
            "price": price_str,
            "shop": "grybezpradu",
            "url": url_prod,

...
def parse_page(html):
    ...
    for tile in soup.select("product-tile"):
        ...
        products.append({"id": f"luznepaczki_{pid}", "name": name, "price": price_str, "shop": SHOP, "url": href, "image": image, "stock": None, "available": available})
    return products
            ...
            if page_html:
                for p in parse_page(page_html):
                    if p["id"] not in seen_ids:
                        seen_ids.add(p["id"])
                        products.append(p)
    print(f"[LUZNEPACZKI] {len(products)} produktow")
    return products
